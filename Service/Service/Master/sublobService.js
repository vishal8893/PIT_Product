...
var routes = function () {
    router.route('/GetAllsubLobMst')
        .get(function (req, res) {
            ...
            var param = {
                where: { IS_ACTIVE: true }, attributes: ['ID', 'BG_ID', 'SBU_ID', 'LOB_ID', 'SUBLOB_NAME', 'SUBLOB_CODE', 'IS_ACTIVE'],order: [['ID', 'DESC']],
                include: [
                    {
                        model: TBL_LOB_MST,
            ...
            dataaccess.FindAll(TBL_SUBLOB_MST, param)
                .then(function (result) {
                    ...
                    dataconn.errorlogger('sublobService', 'GetAllLobMst', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of TBL_SUBLOB_MST Table', Data: null });
                });
    ...
    router.route("/CheckDuplicateSubLob/:Value/:Id")
        .get(function (req, res) {
            ...
            dataaccess.FindAll(TBL_SUBLOB_MST, param).then(
                function (result) {
                    if (
                        result != null &&
                        result.length > 0 &&
                        ...
                        res
                            ...
                            .json({
                                Success: true,
                                Message: "SUB LOB already exists",
                                Data: true,
                            });
                    } else {
                        res
                            ...
                            .json({
                                Success: false,
                                Message: "SUB LOB does not exists",
                                Data: false,
                            });
                    }
                },
                function (err) {
                    ...
                    res
                        ...
                        .json({
                            Success: false,
                            Message: "User has no access of SubLob",
                            Data: null,
                        });
                }
            );
    ...
    router.route('/CreateSubLobMst')
        .post(function (req, res) {
            ...
            var values = {
                ...
                SUBLOB_NAME: encryptmodel.SUBLOB_NAME,
                SUBLOB_CODE: encryptmodel.SUBLOB_CODE,
                IS_ACTIVE: true,
                Created_By: encryptmodel.UserId,
                // Created_Date: connect.sequelize.fn("NOW"),
            };
            console.log("values", values);
            dataaccess.Create(TBL_SUBLOB_MST, values)
                .then(function (result) {
                    if (result != null) {
                        var EncryptLoginDetails = dataconn.encryptionAES(result);
                    ...
                    dataconn.errorlogger('sublobService', 'CreateSubLobMst', err);
                    res.status(200).json({ Success: false, Message: 'Error occurred while saving record', Data: null });
                });
        });
    router.route('/UpdateSubLobMst')
        .post(function (req, res) {
            ...
            dataaccess.Update(TBL_SUBLOB_MST, values, param)
                .then(function (result) {
                    if (result != null) {
                        var EncryptLoginDetails = dataconn.encryptionAES(result);
                        ...
                        dataconn.errorlogger('sublobService', 'UpdateSubLobMst', { message: 'No object found', stack: '' });
                        res.status(200).json({ Success: false, Message: 'Error occurred while updating record', Data: null });
                    }
                }, function (err) {
                    dataconn.errorlogger('sublobService', 'UpdateLobMst', err);
                    res.status(200).json({ Success: false, Message: 'Error occurred while updating record', Data: null });
                });
        });
        ...
        .get(function (req, res) {
            ...
            dataaccess.FindAll(TBL_SBU_MST, param)
                .then(function (result) {
                    ...
                    dataconn.errorlogger('sublobService', 'GetAllSBU', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of TBL_SBU_MST Table', Data: null });
                });
    ...
    router.route('/GetSUBLOB_ID/:ID')
        .get(function (req, res) {
            ...
            dataaccess.FindOne(TBL_SUBLOB_MST, param)
                .then(function (result) {
                    console.log("result", result);
                    if (result != null) {
                        var EncryptLoginDetails = dataconn.encryptionAES(result);
                        res.status(200).json({ Success: true, Message: 'SUBLob Data', Data: EncryptLoginDetails });
                    }
                    else {
                        res.status(200).json({ Success: false, Message: 'User Has No Access Of SUBLOB_MST Table', Data: null });
                    }
                }, function (err) {
                    dataconn.errorlogger('sublobService', 'GetSUBLOB_ID', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of SUBLOB_MST Table', Data: null });
                });
    ...
    router.route('/DeletesubLOBById')
        .post(function (req, res) {
            ...
            console.log("param", param);
            const TBL_SUBLOB_MST = datamodel.TBL_SUBLOB_MST();
            dataaccess.Update(TBL_SUBLOB_MST,{IS_ACTIVE:false}, param)
                .then(function (result) {
                    if (result != null) {
                    ...
                    dataconn.errorlogger('subLOBService', 'DeletesubLOBById', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of TBL_SUBLOB_MST Table', Data: null });
                });
        });
    ...
    router.route('/GetAllEmpgroupMst')
        ...
        .post(function (req, res) {
            ...
            var encryptmodel = dataconn.decrypt(req.body.encryptmodel);
            const BranchMst = datamodel.TBL_SUBLOB_MST();
            // console.log(req.body.SUBLOB_CODE);
            ...
            function (result) {
                if (
                    ...
                    res
                        ...
                        .json({
                            Success: true,
                            Message: "SUBLOB_CODE already exists",
                            Data: true,
                        });
                } else {
                    res
                        ...
                        .json({
                            Success: false,
                            Message: "SUBLOB_CODE does not exists",
                            Data: false,
                        });
                }
            },
