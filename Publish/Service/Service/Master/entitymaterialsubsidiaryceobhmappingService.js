...
var routes = function () {
        ...
        .get(function (req, res) {
            ...
            dataaccess.FindAll(TBL_USER_MST, param)
                .then(function (result) {
                    ...
                    dataconn.errorlogger('entitymstsubceobhmappingservice', 'GetAllEmpgroupMst', err);
    ...
    router.route('/GetAllentitylist')
        .post(function (req, res) {
            ...
            const TBL_ENTITY_MST = datamodel.TBL_ENTITY_MST();
            var param = {
                where: { EMPLOYEE_ID: encryptmodel.EMPLOYEE_ID,IS_ACTIVE: true },
                attributes: ['ID', 'ENTITY_ID', 'EMPLOYEE_ID','EMP_NAME'],
                include: [{ model: TBL_ENTITY_MST, attributes: ['ID', 'ENTITY_NAME'] }]
            };

            dataaccess.FindAll(TBL_ENTITY_CEO_BH_MAPPING_MST, param)
                .then(function (result) {
                    if (result != null) {
                        console.log("GetAllEntity", result)
                        var EncryptLoginDetails = dataconn.encryptionAES(result);
                    ...
                    dataconn.errorlogger('entitymstsubceobhmappingservice', 'GetAllentitylist', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of TBL_ENTITY_CEO_BH_MAPPING_MST Table', Data: null });
                });

        });

    router.route('/GetAllmateriallist')
        .get(function (req, res) {
            ...
            dataaccess.FindAll(TBL_MATERIALSUBSIDIARY_MST, param)
                .then(function (result) {
                    ...
                    dataconn.errorlogger('entitymstsubceobhmappingservice', 'GetAllmateriallist', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of TBL_MATERIALSUBSIDIARY_MST Table', Data: null });
                });
        });

    router.route('/CreatEntMatSubCEOBH')
        .post(function (req, res) {
            var encryptmodel = dataconn.decrypt(req.body.encryptmodel); 
            const TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST = datamodel.TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST();
            var values = {
                ...
                MATERIAL_ID: encryptmodel.MATERIAL_ID,
                IS_ACTIVE: true,

            };
            dataaccess.Create(TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST, values)
                .then(function (result) {
                    if (result != null) {
                        var EncryptLoginDetails = dataconn.encryptionAES(result);
                        res.status(200).json({ Success: true, Message: 'EntityMatSubCEOBMapping saved successfully', Data: EncryptLoginDetails });
                    }
                    else {
                    ...
                    dataconn.errorlogger('entitymstsubceobhmappingservice', 'CreatEntMatSubCEOBH', err);
                    res.status(200).json({ Success: false, Message: 'Error occurred while saving record', Data: null });
                });
        });

    router.route('/UpdateEntMatSubCEOBH')
        .post(function (req, res) {
            var encryptmodel = dataconn.decrypt(req.body.encryptmodel);
            const TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST = datamodel.TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST();
            var values = {
            ...
            dataaccess.Update(TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST, values, param)
                .then(function (result) {
                    if (result != null) {
                        var EncryptLoginDetails = dataconn.encryptionAES(result);
                        res.status(200).json({ Success: true, Message: 'TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST updated successfully', Data: EncryptLoginDetails });
                    }
                    else {
                    ...
                    dataconn.errorlogger('entitymstsubceobhmappingservice', 'UpdateEntMatSubCEOBH', err);
                    res.status(200).json({ Success: false, Message: 'Error occurred while updating record', Data: null });
                });
        });

    router.route('/DeleteEntMatSubCEOBH')
        .post(function (req, res) {
            ...
            console.log("param", param);
            const TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST = datamodel.TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST();
            dataaccess.Update(TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST,{IS_ACTIVE:false}, param)
                .then(function (result) {
                    if (result != null) {
                    ...
                    dataconn.errorlogger('entitymstsubceobhmappingservice', 'DeleteEntMatSubCEOBH', err);
                    res.status(200).json({ Success: false, Message: 'User Has No Access Of TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST Table', Data: null });
                });
        });

    router.route("/CheckDuplicateEntMatSubCEOBH/:Value/:Id")
        .get(function (req, res) {
            const TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST = datamodel.TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST();
            var param = {
                where: {
                ...
                attributes: [
                    [connect.sequelize.fn("count", connect.sequelize.col("ENTITY_ID", "MATERIAL_ID")), "Count",],
                ],
            };

            dataaccess.FindAll(TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST, param).then(
                function (result) {
                    ...
                    dataconn.errorlogger("entitymstsubceobhmappingservice", "CheckDuplicateEntMatSubCEOBH", err);
                    res.status(200).json({ Success: false, Message: "User has no access of entitymstsubceobhmappingservice", Data: null, });
                }
            );
        });
    ...
    router.route('/CheckDuplicate')
        .post(function (req, res) {
            ...
            var encryptmodel = dataconn.decrypt(req.body.encryptmodel);
            const BranchMst = datamodel.TBL_ENTITY_MATERIALSUBSIDIARY_CEO_BH_MAPPING_MST();
            var param = {
                where: {
                    EMPLOYEE_ID: encryptmodel.EMPLOYEE_ID,
