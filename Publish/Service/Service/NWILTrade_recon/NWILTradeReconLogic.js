...
async function intraday_check(Data, date) {
    ...
    try {
        ...
        const query = `
            ...
            FROM
            temp_data AS t1
            INNER JOIN temp_data AS t2 ON t1.EmpId = t2.EmpId AND t1.ISIN = t2.ISIN AND t1.Mode = t2.Mode
            WHERE
            ...
            temp_data
            WHERE
                Mode = 'Buy' AND TradeDate:: DATE = '${date}' AND ISIN IS NOT NULL
            GROUP BY EmpId,ISIN;
            ...
            temp_data
            WHERE
                Mode = 'Sell' AND TradeDate:: DATE = '${date}' AND ISIN IS NOT NULL
            GROUP BY EmpId,ISIN;
            ...
            SELECT t.EmpId, t.TradeDate, t.ScripCode, t.ScripName, t.ISIN, t.Quantity, t.TotalPrice, t.Mode,
                CASE
                    ...
                    CASE
                        WHEN t.Mode = 'Sell' THEN 'Intraday Violation'
                    END 
                END AS Voilation,
            ...
            (
                ...
                SELECT sos.TransId, sos.TradeDate::DATE, sos.EmpId, sos.Exch, sos.ISIN, sos.Quantity, sos.TotalPrice, sos.Mode, temp."sumTotalPrice",
                temp."sumQuantity", UM."EFSLDESIGNATED", sos.ScripCode, sos.ScripName
                FROM
                temp_1 temp
                INNER JOIN temp_data sos ON temp.TransId = sos.TransId
                INNER JOIN "TBL_USER_MST" UM ON UM."EMPNO" = sos.EmpId
                ...
                --temp."sumQuantity",
                --CASE
                    --WHEN sos.ISIN = 'INE531F01015' THEN UM."EFSLDESIGNATED"
                    --ELSE UM."DSIGNATED"
                --END AS Designated,
                --sos.ScripCode, sos.ScripName
                --FROM temp_1 temp
                --INNER JOIN temp_data sos ON temp.TransId = sos.TransId
                --INNER JOIN "TBL_USER_MST" UM ON UM."EMPNO" = sos.EmpId
                --WHERE
                    --(sos.ISIN <> 'INE531F01015') OR (sos.ISIN = 'INE531F01015')
            ) t
...
async function noApprovalCases(date, thresholdAmount, Data, Data4) {
    try {
        ...
        const query = ` 
                       ...
                       DROP TABLE TEMP_EIRF_Rico_SOS_Processed; 
                        ...
                        t."sumQuantity", UM."DSIGNATED" AS Designated, sos.ScripCode, sos.ScripName
                        FROM (
                            SELECT SUM(t2.TotalPrice) AS "sumTotalPrice", SUM(t2.Quantity) AS "sumQuantity", MIN(t1.TradeDate) AS "TradeDate", t1.TransId
                            FROM temp_data t1
                            INNER JOIN temp_data t2 ON t1.EmpId = t2.EmpId AND t1.ISIN = t2.ISIN AND t1.Mode = t2.Mode
                        ...
                        SELECT sos.TransId, sos.TradeDate, sos.EmpId, sos.Exch, sos.ISIN, sos.Quantity, sos.TotalPrice, sos.Mode, t."sumTotalPrice",
                        t."sumQuantity", UM."EFSLDESIGNATED" AS Designated, sos.ScripCode, sos.ScripName
                        FROM (
                            SELECT SUM(t2.TotalPrice) AS "sumTotalPrice", SUM(t2.Quantity) AS "sumQuantity", MIN(t1.TradeDate) AS "TradeDate", t1.TransId
                            FROM temp_data t1
                            INNER JOIN temp_data t2 ON t1.EmpId = t2.EmpId AND t1.ISIN = t2.ISIN AND t1.Mode = t2.Mode
                        ...
                        INNER JOIN temp_data sos ON t.TransId = sos.TransId
                        INNER JOIN "TBL_USER_MST" UM ON UM."EMPNO" = sos.EmpId
                        WHERE (UM."EFSLDESIGNATED" = true OR t."sumTotalPrice" >= '${thresholdAmount}') AND sos.ISIN = 'INE531F01015'
                        ORDER BY EmpId, ISIN, Mode, TotalPrice, Designated;
                        ...
                        FROM (
                            ...
                            INNER JOIN "TBL_USER_MST" ON "EMPNO" = "EmployeeNumber"
                            WHERE
                                ("CurrentTradeValue_Greater":: integer = 1 OR "EFSLDESIGNATED" = true OR "PreviousTradeValueGreater":: integer = 1) AND
                                "CRE_DATE"::DATE = '${date}' AND
                                "ISIN" IS NOT NULL AND
                                "NatureofTrade" IN ('Equity','Debt') AND
                                "ISIN" = 'INE531F01015' AND -- Nuvama scrips
                                "ApprovalStatus" = 'Approved' AND
                                ("IEApprovalStatus" = 'Approved' OR "IEApprovalStatus" IS NULL)
                            GROUP BY "Transaction", "EmployeeNumber", "ISIN"
                        ...
                        INNER JOIN (
                            SELECT
                                ...
                                FROM tempSOS
                                WHERE
                                    Designated = false AND
                                    "sumTotalPrice" >= '${thresholdAmount}' AND
                                    Mode = 'Sell'
                                    GROUP BY ISIN, EmpId, TradeDate::DATE
                        ) sos ON sos.EmpID = EIRF."EmployeeNumber" AND sos.ISIN = EIRF."ISIN" AND EIRF."Transaction" = 'Sell'
                        WHERE
                            "ApprovalStatus" = 'Approved' AND
                            ("IEApprovalStatus" = 'Approved' OR "IEApprovalStatus" IS NULL) AND
                            UM."DSIGNATED" = false AND
                            "PreviousTradeValueGreater":: integer = 0 AND
                            EIRF."Transaction" = 'Sell' AND
                            "CRE_DATE"::DATE = '${date}'
    ...
    --Create temp table if not exists
                            ...
                            SELECT
                                ...
                                sos.TransId,eirf."TRX_NO",eirf.ApprovalDate,eirf.Quantity AS EirfQuantity
                            FROM tempEIRF eirf
                            FULL JOIN tempSOS sos ON eirf.EmpId = sos.EmpId AND eirf.ISIN = sos.ISIN AND eirf.Mode = sos.Mode AND eirf."CRE_DATE":: DATE = sos.TradeDate:: DATE
                            ); 
...
async function restrictedlistCheck(Data, date, Data5) {
    try {
        ...
        const query = `
                        ...
                        LEFT JOIN "TBL_RESTRICTED_LIST_MST" res ON res."ISIN" = SOS.ISIN
                        AND "IS_ACTIVE"= true AND '${date}' BETWEEN res."STARTDATE"::DATE AND COALESCE(res."ENDDATE"::DATE, '${date}')
                        WHERE EMP."EFSLDESIGNATED" = true AND SOS.TradeDate::DATE = '${date}'
                        AND SOS.ISIN = 'INE531F01015';
...
async function UPSIProjectCheck(Data, date, Data7) {
    try {
        ...
        const query = `
                    ...
                    WHERE PD."IS_ACTIVE" = true AND PE."IS_ACTIVE" = true AND SE."IS_ACTIVE" = true AND (SE."ISIN" <> 'INE531F01015' OR SE."ISIN" = 'INE531F01015')
                    ...
                    AND (SE."ISIN" <> 'INE531F01015' OR SE."ISIN" = 'INE531F01015')
                    ...
                    PE."IS_ACTIVE" = true AND SE."IS_ACTIVE" = true AND (SE."ISIN" <> 'INE531F01015' OR SE."ISIN" = 'INE531F01015')
                    ...
                    AND (SOS.ISIN <> 'INE531F01015' OR SOS.ISIN = 'INE531F01015');
    ...
    try {
        ...
        const query = `
                    ...
                    WHERE 
                        ISIN IS NOT NULL
                        AND Mode = 'Buy' 
                        AND (ISIN <> 'INE531F01015' OR ISIN = 'INE531F01015')
                        AND OpenQuantity > 0 
                    ...
                    SELECT
                        ...
                        t1.TransId
                    FROM temp_4 t1
                    INNER JOIN temp_5 t2 ON t1.EmpId = t2.EmpId AND t1.ISIN = t2.ISIN AND t1.Mode = t2.Mode
                    GROUP BY t1.TransId;
                    ...
                    SELECT 
                        ...
                        'Sell' as Mode,map.TransId as TransId,temp.sumTotalPrice,UM."DSIGNATED" as Desg
                    FROM temp_data map
                    ...
                    SELECT 
                        map.EmpId,map.TradeDate AS TradeDate,map.ISIN,map.Quantity as Quantity,map.TotalPrice as TotalPrice,
                        'Sell' as Mode,map.TransId as TransId,temp.sumTotalPrice,UM."EFSLDESIGNATED" as Desg
                    FROM temp_data map
                    INNER JOIN temp_6 as temp ON map.TransId = temp.TransId
                    INNER JOIN "TBL_USER_MST" UM ON UM."EMPNO" = map.EmpId
                    WHERE
                        ...
                        AND map.Mode = 'Sell' 
                        AND map.TradeDate:: date = '${date}'
                        AND UM."EFSLDESIGNATED" = false
                        AND map.ISIN = 'INE531F01015'
                    ORDER BY TradeDate;
                    ...
                    SELECT
                        map.EmpId,map.TradeDate AS TradeDate,map.ISIN,map.Quantity as Quantity,map.TotalPrice as TotalPrice,
                        'Sell' as Mode,map.TransId as TransId,temp.sumTotalPrice,UM."DSIGNATED" as desig
                    FROM temp_data map
                    INNER JOIN temp_6 as temp ON map.TransId = temp.TransId
                    INNER JOIN "TBL_USER_MST" UM ON UM."EMPNO" = map.EmpId
                    WHERE
                    ...
                    SELECT
                        map.EmpId,map.TradeDate AS TradeDate,map.ISIN,map.Quantity as Quantity,map.TotalPrice as TotalPrice,
                        'Sell' as Mode,map.TransId as TransId,temp.sumTotalPrice,UM."EFSLDESIGNATED" as desig
                    FROM temp_data map
                    INNER JOIN temp_6 as temp ON map.TransId = temp.TransId
                    INNER JOIN "TBL_USER_MST" UM ON UM."EMPNO" = map.EmpId
                    WHERE
                    ...
                    WHERE
                        (UM."DSIGNATED" = false OR (UM."EFSLDESIGNATED" = false AND buy.ISIN = 'INE531F01015'))
...
async function NWILRicoReconLogic(date) {
    try {
        ...
        const errMsg = error.message;
        console.error('Error in NWILRicoReconLogic:', errMsg);
...
module.exports.NWILRicoReconLogic = NWILRicoReconLogic;
